...
const renderCard = (auction) => {
  ...
  return `
    <a href="#/auction/${auction.id}" class="group relative bg-[#0e0e1a]/40 border border-white/5 rounded-[32px] md:rounded-[48px] p-4 md:p-5 transition-all duration-700 hover:border-accent-blue/40 hover:-translate-y-2 hover:shadow-premium ring-1 ring-white/5 hover:ring-white/10 block no-underline text-white cursor-pointer">
      <div class="aspect-[4/3] rounded-[24px] md:rounded-[36px] overflow-hidden relative mb-6 md:mb-8 shadow-2xl">
        ...
        <div class="absolute top-3 md:top-5 right-3 md:right-5 ${isLive ? (time.urgent ? 'bg-rose-500/30 border-rose-500/40' : 'bg-black/40 border-white/10') : 'bg-white/10 border-white/10'} backdrop-blur-xl px-3 md:px-5 py-1.5 md:py-2 rounded-full text-[8px] md:text-[9px] font-black text-white uppercase tracking-[0.2em] border ${isLive ? 'animate-pulse' : ''}" data-timer="${auction.id}">
      ...
      <div class="px-2 md:px-3 pb-2 md:pb-3">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 pt-2">
          <div class="space-y-1">
            ...
            <p class="text-xl md:text-2xl font-black font-outfit text-white tracking-tighter" data-bid="${auction.id}">$${auction.currentBid.toLocaleString()}</p>
          </div>
          <span class="bg-white/5 hover:bg-white text-white hover:text-black transition-all px-5 md:px-6 py-2 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest border border-white/10 whitespace-nowrap">
            View Auction
          </span>
        </div>
      </div>
...
const BrowseView = () => {
  ...
  return `
  <section class="max-w-[1440px] mx-auto px-6 md:px-10 py-16 md:py-20 flex flex-col lg:flex-row gap-12 md:gap-16">
    <!-- Sidebar Filters -->
    <aside class="w-full lg:w-80 shrink-0 space-y-12 md:space-y-16">
      <div>
    ...
    <div class="flex-grow">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 md:gap-12" id="auction-grid">
